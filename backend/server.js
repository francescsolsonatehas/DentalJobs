...
app.get("/stats/candidatos-interesados-lista/:empresa_id", verifyToken, (req, res) => {
  db.all(
    `SELECT c.id, c.usuario_id, c.estado, c.mensaje, c.creado_en,
     ...
     -- a una suplencia o colaboración quedarían fuera de "Postulaciones Recibidas".
...
// el chat está abierto a todos los miembros de la plataforma, sin necesidad de una
...
app.post("/archivos/upload", verifyToken, subirArchivo, (req, res) => {
  ...
  const guardarArchivo = async () => {
    try {
      // Las imágenes se recomprimen a WebP (ver imagenes.js) y los PDF con
      // Ghostscript (ver pdfs.js) antes de guardarse; el resto (adjuntos de chat que
      // no son ni imagen ni PDF) se guarda tal cual. Si la recompresión falla o no
